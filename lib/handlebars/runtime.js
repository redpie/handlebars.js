...
Handlebars.VM = {
  ...
  evaluateProperty: function(context, data, parts) {
    // console.log('Evaluating Property: ', parts, context, data);
    ...
    for(var i=0, l=parts.length; i<l; i++) {
        ...
        if (part[0] == '{' && part[part.length-1] === '}') {
            ...
            part = this.evaluateProperty(context, data, part.slice(1,-1).split('.'));
            effectiveContext = this.evaluateProperty(effectiveContext, data, part.split('.'));
        } else {
            effectiveContext = Handlebars.VM.nameLookup(effectiveContext, part);
        }
    }

    // console.log('Obtained Result: ', parts, effectiveContext);
